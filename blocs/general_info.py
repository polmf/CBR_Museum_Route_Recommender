...
def render():
    ...
    if st.session_state.companyia == "Group":
        st.session_state.group_size = ask_question_numerical("How many people are you visiting with?", 1, 10)
    st.session_state.dies = ask_question_numerical("How many days are you planning on coming to the museum?", 1, 10)
    st.session_state.hores = ask_question_numerical("How many hours do you want to spend each day?", 1, 12)
